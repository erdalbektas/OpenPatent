...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run openpatent",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for openpatent session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found openpatent session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`openpatent import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting openpatent...")
        UI.println()

        // Launch openpatent TUI with session ID if available
        const { spawn } = await import("child_process")
        const openpatentArgs = sessionId ? ["-s", sessionId] : []
        const openpatentProcess = spawn("openpatent", openpatentArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          openpatentProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`openpatent exited with code ${code}`))
          })
          openpatentProcess.on("error", reject)
        })
      },
    })
  },
