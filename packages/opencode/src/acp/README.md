...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for openpatent.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal openpatent sessions
...
openpatent acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via openpatent's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | openpatent acp
```
...
### Mapping to openpatent

ACP sessions map cleanly to openpatent's internal session model:
