import { Slug } from "@openpatent/util/slug"
...
import { OpenPatentUsage } from "../auth/openpatent"
...
export namespace Session {
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".openpatent", "plans")
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      // It looks like OpenPatent's cost calculation assumes all providers return inputTokens the same way Anthropic does (I'm guessing getUsage logic was originally implemented with anthropic), so it's causing incorrect cost calculation for OpenRouter and others.
