...
function globalFiles() {
  ...
  if (Flag.OPENPATENT_CONFIG_DIR) {
    files.push(path.join(Flag.OPENPATENT_CONFIG_DIR, "PATENT.md"))
  }
  files.push(path.join(Global.Path.config, "PATENT.md"))
  if (!Flag.OPENPATENT_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.OPENPATENT_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no OPENPATENT_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.OPENPATENT_CONFIG_DIR, Flag.OPENPATENT_CONFIG_DIR).catch(() => [])
}
