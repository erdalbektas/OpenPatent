...
const TUI_SCHEMA_URL = "https://openpatent.ai/tui.json"
...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.OPENPATENT_DISABLE_PROJECT_CONFIG
    ? []
  ...
  if (Flag.OPENPATENT_CONFIG) files.push(Flag.OPENPATENT_CONFIG)
