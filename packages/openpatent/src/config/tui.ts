...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.OPENPATENT_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.OPENPATENT_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.OPENPATENT_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".openpatent") && dir !== Flag.OPENPATENT_CONFIG_DIR) continue
