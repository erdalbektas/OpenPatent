...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .openpatent/ subdirectory too)
  const candidates = [path.join(baseDir, "openpatent.json"), path.join(baseDir, "opencode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".openpatent", "openpatent.json"), path.join(baseDir, ".openpatent", "opencode.jsonc"))
  }
