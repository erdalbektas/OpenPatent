...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@openpatent/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.OPENPATENT_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: OpencodeClient) {
      ...
      const agent = await (async () => {
        ...
        try {
          ...
          if (error.name === "AgentPlanError") {
            UI.println("")
            UI.println(UI.Style.TEXT_DANGER_BOLD + "Premium Agent Required")
            UI.println(UI.Style.TEXT_NORMAL + "")
            UI.println(`The agent "${args.agent}" requires a ${error.requiredPlan} plan.`)
            UI.println("")
            UI.println(UI.Style.TEXT_INFO_BOLD + "Your options:")
            UI.println(UI.Style.TEXT_NORMAL + "  1. Run 'openpatent auth openpatent login' to sign in")
            UI.println(UI.Style.TEXT_NORMAL + "  2. Visit https://openpatent.techtank.com.tr to upgrade")
            UI.println(UI.Style.TEXT_NORMAL + "  3. Run 'openpatent agent list' to see available agents")
            UI.println("")
            process.exit(1)
          }
          throw error
