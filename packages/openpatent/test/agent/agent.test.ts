...
test("returns default native agents when no config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(names).toContain("prior-art")
...
test("build agent has correct default properties", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const build = await Agent.get("draft")
...
test("plan agent denies edits except .openpatent/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const plan = await Agent.get("strategy")
      expect(plan).toBeDefined()
      // Wildcard is denied
      expect(evalPerm(plan, "edit")).toBe("deny")
      // But specific path is allowed
      expect(PermissionNext.evaluate("edit", ".openpatent/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("explore agent denies edit and write", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const explore = await Agent.get("prior-art")
...
test("general agent denies todo tools", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const general = await Agent.get("analyst")
...
test("agent disable removes agent from list", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const agents = await Agent.list()
      const names = agents.map((a) => a.name)
      expect(names).not.toContain("prior-art")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".openpatent", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await Agent.get("draft")
        const skillDir = path.join(tmp.path, ".openpatent", "skill", "perm-skill")
...
test("defaultAgent throws when default_agent points to subagent", async () => {
  await using tmp = await tmpdir({
    config: {
      default_agent: "prior-art",
    },
  })
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      await expect(Agent.defaultAgent()).rejects.toThrow('default agent "prior-art" is a subagent')
    },
  })
})
