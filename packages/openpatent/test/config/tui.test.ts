...
afterEach(async () => {
  delete process.env.OPENPATENT_CONFIG
  delete process.env.OPENPATENT_TUI_CONFIG
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".openpatent"), { recursive: true })
      await Bun.write(
        path.join(dir, ".openpatent", "tui.json"),
...
test("migrates tui-specific keys from opencode.json when tui.json does not exist", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "openpatent.json")))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "openpatent.json"), JSON.stringify({ theme: "legacy" }, null, 2))
...
test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "openpatent.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "openpatent.json")
...
test("migrates legacy tui keys across multiple opencode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "openpatent.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "openpatent.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over OPENPATENT_TUI_CONFIG (matches OPENPATENT_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.OPENPATENT_TUI_CONFIG = custom
    },
  })
...
test("OPENPATENT_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from OPENPATENT_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "openpatent.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.OPENPATENT_CONFIG = path.join(customDir, "openpatent.json")
    },
  })
...
test("loads .openpatent/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".openpatent"), { recursive: true })
      await Bun.write(path.join(dir, ".openpatent", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
