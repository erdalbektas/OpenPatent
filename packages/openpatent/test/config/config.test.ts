...
const managedConfigDir = process.env.OPENPATENT_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "openpatent.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openpatent.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openpatent.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openpatent.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .openpatent directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openpatent")
...
test("loads agents from .openpatent/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openpatent")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openpatent/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openpatent")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .openpatent/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".openpatent")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only OPENPATENT_CONFIG_DIR", async () => {
  ...
  const prev = process.env.OPENPATENT_CONFIG_DIR
  process.env.OPENPATENT_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENPATENT_CONFIG_DIR
    else process.env.OPENPATENT_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENPATENT_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const opencodeDir = path.join(projectDir, ".openpatent")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Filesystem.write(
        path.join(opencodeDir, "openpatent.json"),
        JSON.stringify({
          $schema: "https://openpatent.ai/config.json",
...
// Note: preload.ts sets OPENPATENT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openpatent config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openpatent directory config enables it
      const opencodeDir = path.join(dir, ".openpatent")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "openpatent.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.openpatent/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.openpatent/plugin/oh-my-opencode.js")
  })
...
describe("OPENPATENT_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"]
    process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openpatent/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"]
    process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openpatent directory with a command
          const opencodeDir = path.join(dir, ".openpatent", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .openpatent should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENPATENT_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENPATENT_CONFIG_DIR"]
      process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENPATENT_CONFIG_DIR"]
      } else {
        process.env["OPENPATENT_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENPATENT_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENPATENT_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENPATENT_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENPATENT_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from OPENPATENT_CONFIG_DIR, not project
...
describe("OPENPATENT_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENPATENT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENPATENT_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENPATENT_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://openpatent.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENPATENT_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENPATENT_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENPATENT_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENPATENT_CONFIG_CONTENT"]
