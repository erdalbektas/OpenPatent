...
import { TooltipKeybind } from "@openpatent/ui/tooltip"
import { ResizeHandle } from "@openpatent/ui/resize-handle"
import { Mark } from "@openpatent/ui/logo"
import { DragDropProvider, DragDropSensors, DragOverlay, SortableProvider, closestCenter } from "@thisbeyond/solid-dnd"
import type { DragEvent } from "@thisbeyond/solid-dnd"
import { ConstrainDragYAxis, getDraggableId } from "@/utils/solid-dnd"
import { useDialog } from "@openpatent/ui/context/dialog"
...
import { DrawingsPanel } from "@/components/drawings/drawings-panel"
...
export function SessionSidePanel(props: {
  ...
  const drawingsOpen = createMemo(() => tabs().active() === "drawings" || tabs().all().includes("drawings"))
  const openedTabs = createMemo(() =>
    tabs()
      .all()
      .filter((tab) => tab !== "context" && tab !== "review" && tab !== "drawings"),
  )

  const activeTab = createMemo(() => {
    const active = tabs().active()
    if (active === "context") return "context"
    if (active === "drawings") return "drawings"
    if (active === "review" && reviewTab()) return "review"
    if (active && file.pathFromTab(active)) return normalizeTab(active)

    const first = openedTabs()[0]
    if (first) return first
    if (drawingsOpen()) return "drawings"
  ...
  createEffect(() => {
    ...
    if (!drawingsOpen() && tabs().all().length === 0) {
      tabs().open("drawings")
    }
  })
              ...
              <Tabs value={activeTab()} onChange={openTab}>
                <div class="sticky top-0 shrink-0 flex">
                  ...
                  >
                        ...
                        closeButton={
                          ...
                          >
                            <IconButton
                              ...
                              variant="ghost"
                              class="h-5 w-5"
                              onClick={() => tabs().close("drawings")}
                              aria-label={language.t("common.closeTab")}
                            />
                          </TooltipKeybind>
                        }
                        hideCloseButton
                        onMiddleClick={() => tabs().close("drawings")}
                      >
                        <div class="flex items-center gap-2">
                          <div>Drawings</div>
                        </div>
                    ...
                    <TooltipKeybind title="Open Drawings" keybind="" class="flex items-center">
                      <IconButton
                        icon="brain"
                        variant="ghost"
                        iconSize="large"
                        class="!rounded-md"
                        onClick={() => tabs().open("drawings")}
                ...
                <Show when={drawingsOpen()}>
                  <Tabs.Content value="drawings" class="flex flex-col h-full overflow-hidden contain-strict">
                    <Show when={activeTab() === "drawings"}>
                      <div class="relative pt-2 flex-1 min-h-0 overflow-hidden">
                        <DrawingsPanel />
                      </div>
                    </Show>
