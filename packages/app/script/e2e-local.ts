...
const openpatentDir = path.join(repoDir, "packages", "openpatent")
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../openpatent/src/util/log")
    const install = await import("../../openpatent/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../openpatent/src/server/server")
    inst = await import("../../openpatent/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`openpatent server listening on http://127.0.0.1:${serverPort}`)
