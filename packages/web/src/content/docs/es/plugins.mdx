---
title: Complementos
description: Escriba sus propios complementos para extender OpenPatent.
---

Los complementos le permiten extender OpenPatent al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenPatent.
...
- `.OpenPatent/plugins/` - Complementos a nivel de proyecto
- `~/.config/OpenPatent/plugins/` - Complementos globales
...
```json title="OpenPatent.json"
{
  ...
  "plugin": ["OpenPatent-helicone-session", "OpenPatent-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/OpenPatent/node_modules/`.
...
1. Configuración global (`~/.config/OpenPatent/OpenPatent.json`)
2. Configuración del proyecto (`OpenPatent.json`)
3. Directorio global de complementos (`~/.config/OpenPatent/plugins/`)
4. Directorio de complementos del proyecto (`.OpenPatent/plugins/`)
...
```json title=".OpenPatent/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenPatent ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".OpenPatent/plugins/my-plugin.ts"
...
```js title=".OpenPatent/plugins/example.js"
...
- `client`: Un cliente SDK OpenPatent para interactuar con la IA.
...
import type { Plugin } from "@OpenPatent-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar OpenPatent.
...
```js title=".OpenPatent/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "OpenPatent"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenPatent, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que OpenPatent lea archivos `.env`:

```javascript title=".OpenPatent/plugins/env-protection.js"
...
```javascript title=".OpenPatent/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a OpenPatent:

```ts title=".OpenPatent/plugins/custom-tools.ts"
import { type Plugin, tool } from "@OpenPatent-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que OpenPatent puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para OpenPatent junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://OpenPatent.ai/docs/sdk) para obtener más detalles.
...
```ts title=".OpenPatent/plugins/compaction.ts"
import type { Plugin } from "@OpenPatent-ai/plugin"
...
```ts title=".OpenPatent/plugins/custom-compaction.ts"
import type { Plugin } from "@OpenPatent-ai/plugin"
