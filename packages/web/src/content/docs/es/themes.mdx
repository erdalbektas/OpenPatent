...
Con OpenPatent puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenPatent usa nuestro propio tema `OpenPatent`.
...
OpenPatent viene con varios temas integrados.
...
- Quiere que OpenPatent coincida con la apariencia de su terminal
...
OpenPatent admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/OpenPatent/themes/*.json` o `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.OpenPatent/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.OpenPatent/themes/*.json`
