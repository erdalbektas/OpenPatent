...
Las habilidades del agente permiten a OpenPatent descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenPatent busca estas ubicaciones:

- Configuración del proyecto: `.OpenPatent/skills/<name>/SKILL.md`
- Configuración global: `~/.config/OpenPatent/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenPatent sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.OpenPatent/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/OpenPatent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.OpenPatent/skills/git-release/SKILL.md` así:
...
OpenPatent enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `OpenPatent.json`:
...
**Para agentes integrados** (en `OpenPatent.json`):
