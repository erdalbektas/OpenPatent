---
title: Servidor
description: Interactuar con el servidor OpenPatent a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `OpenPatent serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente OpenPatent puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `OpenPatent.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `OpenPatent`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `OpenPatent serve` como a `OpenPatent web`.
...
Cuando ejecuta `OpenPatent`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor OpenPatent para interactuar con OpenPatent mediante programación.
:::

Esta arquitectura permite que OpenPatent admita múltiples clientes y le permite interactuar con OpenPatent mediante programación.

Puede ejecutar `OpenPatent serve` para iniciar un servidor independiente. Si tienes el
OpenPatent TUI ejecutándose, `OpenPatent serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenPatent [IDE](/docs/ide).
...
El servidor OpenPatent expone las siguientes API.
