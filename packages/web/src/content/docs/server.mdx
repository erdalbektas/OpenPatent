---
title: Server
description: Interact with OpenPatent server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `OpenPatent serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an OpenPatent client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `OpenPatent`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `OpenPatent serve` and `OpenPatent web`.
...
When you run `OpenPatent` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the OpenPatent server to interact with OpenPatent programmatically.
:::

This architecture lets OpenPatent support multiple clients and allows you to interact with OpenPatent programmatically.

You can run `OpenPatent serve` to start a standalone server. If you have the
OpenPatent TUI running, `OpenPatent serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenPatent [IDE](/docs/ide) plugins.
...
The OpenPatent server exposes the following APIs.
