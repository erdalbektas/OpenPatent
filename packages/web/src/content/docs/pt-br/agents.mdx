...
Existem dois tipos de agentes no OpenPatent; agentes primários e subagentes.
...
OpenPatent vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
OpenPatent vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

OpenPatent vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `OpenPatent.json`:
...
Se nenhuma temperatura for especificada, o OpenPatent usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do OpenPatent quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do OpenPatent usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenPatent Zen](/docs/zen), você usaria `OpenPatent/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `OpenPatent models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/OpenPatent).
:::
