...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do OpenPatent. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no OpenPatent permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do OpenPatent.
...
O OpenPatent vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.OpenPatent/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `OpenPatent.json`:
...
- Global: `~/.config/OpenPatent/modes/`
- Projeto: `.OpenPatent/modes/`

```markdown title="~/.config/OpenPatent/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o OpenPatent usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do OpenPatent quanto para a configuração específica do projeto.
...
```json title="OpenPatent.json" {4-14}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "mode": {
    "docs": {
...
Crie arquivos de modo em `.OpenPatent/modes/` para modos específicos do projeto ou `~/.config/OpenPatent/modes/` para modos globais:

```markdown title=".OpenPatent/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/OpenPatent/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
