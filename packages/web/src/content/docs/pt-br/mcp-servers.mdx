...
Você pode adicionar ferramentas externas ao OpenPatent usando o _Model Context Protocol_, ou MCP. O OpenPatent suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do OpenPatent](https://OpenPatent.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/OpenPatent`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O OpenPatent lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o OpenPatent irá:
...
Se o servidor requer autenticação, o OpenPatent solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `OpenPatent mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o OpenPatent armazenará os tokens de forma segura em `~/.local/share/OpenPatent/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no OpenPatent, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do OpenPatent como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o OpenPatent à sua conta do Sentry.
