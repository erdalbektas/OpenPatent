...
description: Crie ferramentas que o LLM pode chamar no OpenPatent.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do OpenPatent, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.OpenPatent/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/OpenPatent/tools/`.
