...
As habilidades do agente permitem que o OpenPatent descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O OpenPatent pesquisa nesses locais:

- Configuração do projeto: `.OpenPatent/skills/<name>/SKILL.md`
- Configuração global: `~/.config/OpenPatent/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o OpenPatent sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.OpenPatent/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/OpenPatent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.OpenPatent/skills/git-release/SKILL.md` assim:
...
O OpenPatent lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `OpenPatent.json`:
...
**Para agentes embutidos** (em `OpenPatent.json`):
