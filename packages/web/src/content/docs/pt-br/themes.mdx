...
Com o OpenPatent, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o OpenPatent usa nosso próprio tema `OpenPatent`.
...
O OpenPatent vem com vários temas embutidos.
...
- Querem que o OpenPatent corresponda à aparência do seu terminal
...
O OpenPatent suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/OpenPatent/themes/*.json` ou `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.OpenPatent/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.OpenPatent/themes/*.json`
