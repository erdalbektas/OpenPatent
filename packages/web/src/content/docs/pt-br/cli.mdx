---
title: CLI
description: Opções e comandos da CLI do OpenPatent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do OpenPatent, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
OpenPatent
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o OpenPatent programaticamente.
...
Inicie a interface de usuário do terminal do OpenPatent.
...
A CLI do OpenPatent também possui os seguintes comandos.

---

### agent

Gerencie agentes para o OpenPatent.
...
Anexe um terminal a um servidor backend do OpenPatent já em execução, iniciado via comandos `serve` ou `web`.

```bash
OpenPatent attach [url]
```

Isso permite usar o TUI com um backend OpenPatent remoto. Por exemplo:
...
O OpenPatent é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `OpenPatent auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

Quando o OpenPatent é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no OpenPatent.
...
Execute o OpenPatent em modo não interativo passando um prompt diretamente.
...
```bash "OpenPatent run"
OpenPatent run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `OpenPatent serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
OpenPatent run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| `--attach`   |       | Anexe a um servidor OpenPatent em execução (por exemplo, http://localhost:4096) |
...
Inicie um servidor OpenPatent sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
OpenPatent serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do OpenPatent sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `OpenPatent`).
...
Gerencie sessões do OpenPatent.
...
Liste todas as sessões do OpenPatent.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do OpenPatent.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do OpenPatent.

```bash
OpenPatent import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do OpenPatent.
...
Inicie um servidor OpenPatent sem cabeça com uma interface web.

```bash
OpenPatent web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o OpenPatent através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `OpenPatent`).
...
Desinstale o OpenPatent e remova todos os arquivos relacionados.
...
Atualiza o OpenPatent para a versão mais recente ou uma versão específica.
...
A CLI do OpenPatent aceita as seguintes flags globais.
...
O OpenPatent pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `OpenPatent`) |
