---
title: Plugins
description: Escreva seus próprios plugins para estender o OpenPatent.
---

Plugins permitem que você estenda o OpenPatent conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do OpenPatent.
...
- `.OpenPatent/plugins/` - Plugins em nível de projeto
- `~/.config/OpenPatent/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/OpenPatent/node_modules/`.
...
1. Configuração global (`~/.config/OpenPatent/OpenPatent.json`)
2. Configuração do projeto (`OpenPatent.json`)
3. Diretório de plugins global (`~/.config/OpenPatent/plugins/`)
4. Diretório de plugins do projeto (`.OpenPatent/plugins/`)
...
O OpenPatent executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do OpenPatent para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o OpenPatent.
...
:::note
Se você estiver usando o aplicativo desktop OpenPatent, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o OpenPatent de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao OpenPatent:
...
O helper `tool` cria uma ferramenta personalizada que o OpenPatent pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o OpenPatent junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://OpenPatent.ai/docs/sdk) para detalhes.
