...
Postoje dvije vrste agenata u OpenPatent: primarni agenti i podagenti.
...
OpenPatent dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenPatent dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenPatent dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `OpenPatent.json`:
...
- Globalno: `~/.config/OpenPatent/agents/`
- Po projektu: `.OpenPatent/agents/`
...
Ako temperatura nije navedena, OpenPatent koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenPatent konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenPatent konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenPatent Zen](/docs/zen), koristili biste `OpenPatent/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `OpenPatent models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/OpenPatent).
:::
