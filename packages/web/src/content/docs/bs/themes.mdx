...
U OpenPatent mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenPatent koristi nasu `OpenPatent` temu.
...
OpenPatent dolazi sa vise ugradenih tema.
...
- Zele da OpenPatent odgovara izgledu njihovog terminala
...
OpenPatent podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/OpenPatent/themes/*.json` ili `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.OpenPatent/themes/*.json`
4. **Trenutni radni direktorij** - `./.OpenPatent/themes/*.json`
