---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenPatent.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenPatent koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenPatent konfiguraciji.
...
u `~/.local/share/OpenPatent/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenPatent
config.
...
OpenPatent Zen je lista modela koje je obezbedio OpenPatent tim koji su bili
testirano i potvrđeno da dobro radi sa OpenPatent. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenPatent Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite OpenPatent i idite na [OpenPatent.ai/auth](https://OpenPatent.ai/auth).
...
Radi kao i svaki drugi provajder u OpenPatent i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s OpenPatent:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete OpenPatent:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `OpenPatent.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenPatent.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi OpenPatent ispravno radio.
   :::
...
Također možete dodati modele kroz svoju OpenPatent konfiguraciju.
...
da pohrani token u OpenPatent auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenPatent koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenPatent
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`OpenPatent.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [OpenPatent-gitlab-auth](https://www.npmjs.com/package/@gitlab/OpenPatent-gitlab-auth).

##### Konfiguracija

Prilagodite putem `OpenPatent.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s OpenPatent:
...
Za korištenje Google Vertex AI s OpenPatent:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće OpenPatent.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem OpenPatenta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [OpenPatent-helicone-session](https://github.com/H2Shami/OpenPatent-helicone-session) da automatski prijavite svaki OpenPatent razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenPatent razgovor naveden kao zasebna sesija.
...
Možete konfigurirati OpenPatent za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati OpenPatent za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati OpenPatent za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenPatent. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/OpenPatent) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenPatent:
...
3. Kopirajte API ključ za korištenje u OpenPatent.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenPatent, morate lokalno povući informacije o modelu:
...
OpenPatent Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenPatent tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenPatent Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenPatent Zen**.

```txt
   /connect
```

3. Unesite svoj OpenPatent API ključ.
...
Također možete dodati dodatne modele putem vaše OpenPatent konfiguracije.
...
5. Također ih možete prilagoditi putem vaše OpenPatent konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju OpenPatent konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s OpenPatent-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `OpenPatent.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenPatent da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `OpenPatent auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenPatent konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj OpenPatent konfiguraciji.
