...
Možete dodati vanjske alate u OpenPatent koristeći _Model Context Protocol_, ili MCP. OpenPatent podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenPatent Config](https://OpenPatent.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/OpenPatent`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenPatent automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenPatent će:
...
Ako server zahtijeva autentifikaciju, OpenPatent će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `OpenPatent mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenPatent sigurno čuva tokene u `~/.local/share/OpenPatent/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenPatent, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenPatent konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenPatent sa vašim Sentry nalogom.
