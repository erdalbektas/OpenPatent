---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenPatent.
---

Dodaci vam omogućavaju da proširite OpenPatent spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenPatent.
...
- `.OpenPatent/plugins/` - Dodaci na nivou projekta
- `~/.config/OpenPatent/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/OpenPatent/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/OpenPatent/OpenPatent.json`)
2. Konfiguracija projekta (`OpenPatent.json`)
3. Globalni direktorij dodataka (`~/.config/OpenPatent/plugins/`)
4. Direktorij dodataka projekta (`.OpenPatent/plugins/`)
...
OpenPatent pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenPatent.
...
:::note
Ako koristite desktop aplikaciju OpenPatent, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite OpenPatent da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u OpenPatent:
...
Pomoćnik `tool` kreira prilagođeni alat koji OpenPatent može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za OpenPatent zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://OpenPatent.ai/docs/sdk) za detalje.
