...
Da biste otklonili probleme s OpenPatent, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\OpenPatent\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `OpenPatent --log-level DEBUG`.

---

## Pohrana

OpenPatent pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/OpenPatent/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\OpenPatent`
...
OpenPatent Desktop pokreće lokalni OpenPatent server (`OpenPatent-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenPatent` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/OpenPatent/OpenPatent.jsonc` (ili `~/.config/OpenPatent/OpenPatent.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/OpenPatent/OpenPatent.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\OpenPatent\OpenPatent.jsonc`
...
OpenPatent također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/OpenPatent/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\OpenPatent\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenPatent mogao ponovo izgraditi.

1. Potpuno zatvorite OpenPatent Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/OpenPatent`
- **Linux**: obrišite `~/.cache/OpenPatent` (ili pokrenite `rm -rf ~/.cache/OpenPatent`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\OpenPatent`

3. Ponovo pokrenite OpenPatent Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenPatent Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `OpenPatent.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, OpenPatent Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenPatent karakteristikama.

---

### Obavještenja se ne prikazuju

OpenPatent Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenPatent u postavkama vašeg OS-a, i
...
1. Zatvorite OpenPatent Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenPatent Desktop aplikacije):

- `OpenPatent.settings.dat` (URL zadanog servera za desktop)
- `OpenPatent.global.dat` i `OpenPatent.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s OpenPatent:
...
Da saznate kojim modelima imate pristup, pokrenite `OpenPatent models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\OpenPatent`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenPatent dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\OpenPatent`

2. Ponovo pokrenite OpenPatent da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenPatent da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenPatent će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
