...
ove ili konfigurirajte svoje putem OpenPatent config.
...
OpenPatent dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.OpenPatent/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `OpenPatent.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/OpenPatent/modes/`
- Projekat: `.OpenPatent/modes/`
...
Ako temperatura nije navedena, OpenPatent koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.OpenPatent/modes/` za specifične načine rada ili `~/.config/OpenPatent/modes/` za globalne načine:
