...
Agent skills omogucavaju OpenPatent da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenPatent pretrazuje ove lokacije:

- Konfiguracija projekta: `.OpenPatent/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/OpenPatent/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenPatent ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.OpenPatent/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/OpenPatent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.OpenPatent/skills/git-release/SKILL.md` ovako:
...
OpenPatent navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `OpenPatent.json`:
...
**Za ugradene agente** (u `OpenPatent.json`):
