---
title: CLI
description: OpenPatent CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenPatent CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
OpenPatent
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa OpenPatent.
...
Pokrenite OpenPatent terminalski korisnički interfejs.
...
OpenPatent CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte OpenPatent agentima.
...
Priključite terminal na već pokrenut OpenPatent backend server pokrenut putem `serve` ili `web` naredbi.

```bash
OpenPatent attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim OpenPatent backend-om. Na primjer:
...
OpenPatent pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `OpenPatent auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

Kada se OpenPatent pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u OpenPatent.
...
Pokrenite OpenPatent u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `OpenPatent serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| `--attach`   |        | Priključite na pokrenuti OpenPatent server (npr. http://localhost:4096)    |
...
Pokrenite OpenPatent headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
OpenPatent serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenPatent-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `OpenPatent`).
...
Upravljajte OpenPatent sesijama.
...
Navedite sve OpenPatent sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše OpenPatent sesije.
...
Uvezite podatke sesije iz JSON datoteke ili OpenPatent dijeljenog URL-a.

```bash
OpenPatent import <file>
```

Možete uvesti iz lokalne datoteke ili OpenPatent dijeljenog URL-a.
...
Pokrenite OpenPatent headless server sa web interfejsom.

```bash
OpenPatent web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenPatent-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `OpenPatent`).
...
Deinstalirajte OpenPatent i uklonite sve povezane datoteke.
...
Ažurira OpenPatent na najnoviju verziju ili određenu verziju.
...
OpenPatent CLI prihvata sljedeće globalne zastavice.
...
OpenPatent se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `OpenPatent`) |
