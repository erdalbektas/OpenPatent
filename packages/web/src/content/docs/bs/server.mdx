---
title: Server
description: Komunicirajte s OpenPatent serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `OpenPatent serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji OpenPatent klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `OpenPatent.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `OpenPatent`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `OpenPatent serve` i za `OpenPatent web`.
...
Kada pokrenete `OpenPatent`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite OpenPatent server za programsku interakciju sa OpenPatent-om.
:::

Ova arhitektura omogucava OpenPatent podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `OpenPatent serve` da startate standalone server. Ako je OpenPatent TUI vec pokrenut, `OpenPatent serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenPatent [IDE](/docs/ide) pluginovi.
...
OpenPatent server izlaže sljedece API-je.
