---
title: SDK
description: Client JS de type sécurisé pour le serveur OpenPatent.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK OpenPatent JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler OpenPatent par programme.
...
Créez une instance de OpenPatent :

```javascript
import { createOpencode } from "@OpenPatent-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `OpenPatent.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@OpenPatent-ai/sdk"

const OpenPatent = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${OpenPatent.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de OpenPatent, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@OpenPatent-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@OpenPatent-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*OpenPatent" },
})
