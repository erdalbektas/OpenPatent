---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenPatent.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenPatent utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenPatent.
...
en `~/.local/share/OpenPatent/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenPatent
configuration.
...
OpenPatent Zen est une liste de modèles fournis par l'équipe OpenPatent qui ont été
testé et vérifié pour fonctionner correctement avec OpenPatent. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenPatent Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez OpenPatent et dirigez-vous vers [OpenPatent.ai/auth](https://OpenPatent.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenPatent et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec OpenPatent :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de OpenPatent :
...
Pour une configuration spécifique au projet ou persistante, utilisez `OpenPatent.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenPatent n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que OpenPatent fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration OpenPatent.
...
pour stocker le jeton dans le stockage d'authentification OpenPatent.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
OpenPatent utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenPatent
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `OpenPatent.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [OpenPatent-gitlab-auth](https://www.npmjs.com/package/@gitlab/OpenPatent-gitlab-auth).

##### Configuration

Personnalisez via `OpenPatent.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec OpenPatent :
...
Pour utiliser Google Vertex AI avec OpenPatent :
...
Définissez-les lors de l'exécution de OpenPatent.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via OpenPatent, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [OpenPatent-helicone-session](https://github.com/H2Shami/OpenPatent-helicone-session) pour enregistrer automatiquement chaque conversation OpenPatent en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenPatent répertoriée comme une session distincte.
...
Vous pouvez configurer OpenPatent pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer OpenPatent pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer OpenPatent pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenPatent. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/OpenPatent) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec OpenPatent :
...
3. Copiez la clé API à utiliser dans OpenPatent.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenPatent, vous devez extraire les informations du modèle localement :
...
OpenPatent Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenPatent. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenPatent Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenPatent Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenPatent API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration OpenPatent.
...
5. Vous pouvez également les personnaliser via votre configuration OpenPatent. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration OpenPatent. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec OpenPatent. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `OpenPatent.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à OpenPatent de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `OpenPatent auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration OpenPatent et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration OpenPatent.
