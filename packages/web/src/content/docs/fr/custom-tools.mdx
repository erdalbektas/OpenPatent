...
description: Créez des outils que le LLM peut appeler dans OpenPatent.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de OpenPatent comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.OpenPatent/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/OpenPatent/tools/`.
...
```ts title=".OpenPatent/tools/database.ts" {1}
...
```ts title=".OpenPatent/tools/math.ts"
import { tool } from "@OpenPatent-ai/plugin"
...
```ts title=".OpenPatent/tools/project.ts" {8}
import { tool } from "@OpenPatent-ai/plugin"
...
```python title=".OpenPatent/tools/add.py"
import sys
...
```ts title=".OpenPatent/tools/python-add.ts" {10}
import { tool } from "@OpenPatent-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".OpenPatent/tools/add.py")
