---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenPatent.
---

Les plugins vous permettent d'étendre OpenPatent en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenPatent.
...
- `.OpenPatent/plugins/` - Plugins au niveau du projet
- `~/.config/OpenPatent/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/OpenPatent/node_modules/`.
...
1. Configuration globale (`~/.config/OpenPatent/OpenPatent.json`)
2. Configuration du projet (`OpenPatent.json`)
3. Répertoire global des plugins (`~/.config/OpenPatent/plugins/`)
4. Répertoire des plugins du projet (`.OpenPatent/plugins/`)
...
OpenPatent exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK OpenPatent pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre OpenPatent.
...
:::note
Si vous utilisez l'application de bureau OpenPatent, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez OpenPatent de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à OpenPatent :
...
L'assistant `tool` crée un outil personnalisé que OpenPatent peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour OpenPatent aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://OpenPatent.ai/docs/sdk) pour plus de détails.
