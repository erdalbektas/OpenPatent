...
Il existe deux types d'agents dans OpenPatent ; agents primaires et sous-agents.
...
OpenPatent est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
OpenPatent est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

OpenPatent est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `OpenPatent.json` :
...
- Global : `~/.config/OpenPatent/agents/`
- Par projet : `.OpenPatent/agents/`

```markdown title="~/.config/OpenPatent/agents/review.md"
---
...
Si aucune température n'est spécifiée, OpenPatent utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale OpenPatent et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration OpenPatent utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenPatent Zen](/docs/zen), vous utiliserez `OpenPatent/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="OpenPatent.json" {3-6,9-12}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "tools": {
    "write": true,
...
```json title="OpenPatent.json" {3-5,8-10}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="OpenPatent.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `OpenPatent models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/OpenPatent).
:::
...
```markdown title="~/.config/OpenPatent/agents/docs-writer.md"
---
...
```markdown title="~/.config/OpenPatent/agents/security-auditor.md"
---
