---
title: Configuration
description: Utilisation de la configuration JSON OpenPatent.
---

Vous pouvez configurer OpenPatent à l'aide d'un fichier de configuration JSON.

---

## Format

OpenPatent prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="OpenPatent.jsonc"
{
...
Par exemple, si votre configuration globale définit `theme: "OpenPatent"` et `autoupdate: true` et que la configuration de votre projet définit `model: "anthropic/claude-sonnet-4-5"`, la configuration finale inclura les trois paramètres.
...
1. **Configuration distante** (à partir de `.well-known/OpenPatent`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/OpenPatent/OpenPatent.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`OpenPatent.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.OpenPatent`** - agents, commandes, plugins
...
:::note
Les répertoires `.OpenPatent` et `~/.config/OpenPatent` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/OpenPatent`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/OpenPatent"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale OpenPatent dans `~/.config/OpenPatent/OpenPatent.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les thèmes, les fournisseurs ou les raccourcis clavier.
...
Ajoutez `OpenPatent.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.

:::tip
Placez la configuration spécifique au projet à la racine de votre projet.
:::

Lorsque OpenPatent démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.OpenPatent`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.OpenPatent`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`OpenPatent.ai/config.json`**](https://OpenPatent.ai/config.json).
...
Vous pouvez configurer les paramètres du serveur pour les commandes `OpenPatent serve` et `OpenPatent web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur OpenPatent.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `OpenPatent.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration OpenPatent via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, OpenPatent essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration OpenPatent via l'option `theme`.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/OpenPatent/agents/` ou `.OpenPatent/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, OpenPatent reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`OpenPatent run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/OpenPatent/commands/` ou `.OpenPatent/commands/`. [En savoir plus ici](/docs/commands).
...
OpenPatent téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, OpenPatent **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent OpenPatent avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.OpenPatent/plugins/` ou `~/.config/OpenPatent/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="OpenPatent.json"
{
  "$schema": "https://OpenPatent.ai/config.json",
  "plugin": ["OpenPatent-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre OpenPatent à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
