...
Avec OpenPatent, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenPatent utilise notre propre thème `OpenPatent`.
...
OpenPatent est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenPatent corresponde à l'apparence de leur terminal
...
OpenPatent prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/OpenPatent/themes/*.json` ou `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.OpenPatent/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.OpenPatent/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/OpenPatent/themes
vim ~/.config/OpenPatent/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .OpenPatent/themes
vim .OpenPatent/themes/my-theme.json
```
...
{
  "$schema": "https://OpenPatent.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
