...
Créez `.OpenPatent/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration OpenPatent ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre OpenPatent [config](/docs/config) :
...
- Par projet : `.OpenPatent/commands/`
...
OpenPatent comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
