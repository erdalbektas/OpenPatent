---
title: CLI
description: Options et commandes de la CLI OpenPatent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI OpenPatent démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
OpenPatent
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec OpenPatent par programme.
...
Démarrez l’interface utilisateur du terminal OpenPatent.
...
La CLI OpenPatent dispose également des commandes suivantes.

---

### agent

Gérer les agents pour OpenPatent.
...
Attachez un terminal à un serveur backend OpenPatent déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
OpenPatent attach [url]
```

Cela permet d'utiliser le TUI avec un backend OpenPatent distant. Par exemple :
...
OpenPatent est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `OpenPatent auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

Lorsque OpenPatent démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans OpenPatent.
...
Exécutez OpenPatent en mode non interactif en transmettant directement un prompt.
...
Vous pouvez également vous connecter à une instance `OpenPatent serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
| `--attach`   |       | Connectez-vous à un serveur OpenPatent en cours d'exécution (par exemple, http://localhost:4096) |
...
Démarrez un serveur OpenPatent sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
OpenPatent serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenPatent sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `OpenPatent`).
...
Liste toutes les sessions OpenPatent.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions OpenPatent.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage OpenPatent.

```bash
OpenPatent import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage OpenPatent.
...
Démarrez un serveur OpenPatent sans interface graphique avec une interface Web.

```bash
OpenPatent web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenPatent via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `OpenPatent`).
...
Désinstallez OpenPatent et supprimez tous les fichiers associés.
...
Met à jour OpenPatent vers la dernière version ou une version spécifique.
...
La CLI OpenPatent prend les flags globaux suivants.
...
OpenPatent peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `OpenPatent`) |
