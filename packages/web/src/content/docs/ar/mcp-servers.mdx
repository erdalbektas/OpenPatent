...
يمكنك إضافة أدوات خارجية إلى OpenPatent باستخدام _Model Context Protocol_، أو MCP. يدعم OpenPatent خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات OpenPatent](https://OpenPatent.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/OpenPatent` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى OpenPatent تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم OpenPatent بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك OpenPatent المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `OpenPatent mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن OpenPatent الرموز بشكل آمن في `~/.local/share/OpenPatent/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل OpenPatent إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات OpenPatent مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط OpenPatent بحساب Sentry الخاص بك.
