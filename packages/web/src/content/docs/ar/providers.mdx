---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenPatent.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenPatent ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenPatent.
...
في `~/.local/share/OpenPatent/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenPatent.
...
OpenPatent Zen هي قائمة نماذج يوفّرها فريق OpenPatent وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenPatent. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenPatent Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر OpenPatent، ثم انتقل إلى [OpenPatent.ai/auth](https://OpenPatent.ai/auth).
...
يعمل مثل أي مزوّد آخر في OpenPatent واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع OpenPatent:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل OpenPatent:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `OpenPatent.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenPatent غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل OpenPatent بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات OpenPatent.
...
تخزين الرمز في مساحة تخزين مصادقة OpenPatent.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenPatent نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenPatent
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`OpenPatent.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [OpenPatent-gitlab-auth](https://www.npmjs.com/package/@gitlab/OpenPatent-gitlab-auth).

##### التهيئة

خصّص عبر `OpenPatent.json`:
...
لاستخدام اشتراك GitHub Copilot مع OpenPatent:
...
لاستخدام Google Vertex AI مع OpenPatent:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر OpenPatent، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [OpenPatent-helicone-session](https://github.com/H2Shami/OpenPatent-helicone-session) لتسجيل كل محادثة OpenPatent تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenPatent مدرجة كجلسة مستقلة.
...
يمكنك ضبط OpenPatent لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط OpenPatent لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط OpenPatent لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenPatent. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/OpenPatent) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenPatent:
...
3. انسخ مفتاح API لاستخدامه في OpenPatent.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenPatent، يجب سحب معلومات النموذج محليا:
...
OpenPatent Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenPatent. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenPatent Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenPatent Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenPatent.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات OpenPatent.
...
5. يمكنك أيضا تخصيصها عبر إعدادات OpenPatent. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات OpenPatent. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع OpenPatent. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `OpenPatent.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenPatent بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `OpenPatent auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات OpenPatent وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات OpenPatent.
