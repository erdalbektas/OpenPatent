...
مع OpenPatent يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenPatent سمتنا `OpenPatent`.
...
يأتي OpenPatent مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenPatent مظهر terminal لديهم
...
يدعم OpenPatent نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/OpenPatent/themes/*.json` أو `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.OpenPatent/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.OpenPatent/themes/*.json`
