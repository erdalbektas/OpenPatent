...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في OpenPatent.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في OpenPatent مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.OpenPatent/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/OpenPatent/tools/`.
