---
title: CLI
description: أوامر وخيارات CLI في OpenPatent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ OpenPatent افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
OpenPatent
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع OpenPatent برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ OpenPatent.
...
يتضمن CLI الخاص بـ OpenPatent أيضا الأوامر التالية.

---

### agent

إدارة وكلاء OpenPatent.
...
إرفاق terminal بخادم الواجهة الخلفية لـ OpenPatent قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
OpenPatent attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ OpenPatent تعمل عن بعد. على سبيل المثال:
...
يعتمد OpenPatent على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `OpenPatent auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

عند تشغيل OpenPatent يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في OpenPatent.
...
تشغيل OpenPatent في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `OpenPatent serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| `--attach`   |         | الإرفاق بخادم OpenPatent قيد التشغيل (مثل http://localhost:4096)    |
...
بدء خادم OpenPatent بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
OpenPatent serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف OpenPatent دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `OpenPatent`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات OpenPatent لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة OpenPatent.

```bash
OpenPatent import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة OpenPatent.
...
بدء خادم OpenPatent بدون واجهة مع واجهة ويب.

```bash
OpenPatent web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى OpenPatent عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `OpenPatent`).
...
إلغاء تثبيت OpenPatent وإزالة جميع الملفات ذات الصلة.
...
تحديث OpenPatent إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر OpenPatent الخيارات العامة التالية.
...
يمكن تهيئة OpenPatent باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `OpenPatent`) |
