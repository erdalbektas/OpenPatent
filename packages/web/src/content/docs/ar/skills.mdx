...
تتيح مهارات الوكيل لـ OpenPatent اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث OpenPatent في هذه المواقع:

- إعدادات المشروع: `.OpenPatent/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/OpenPatent/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج OpenPatent صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.OpenPatent/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/OpenPatent/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.OpenPatent/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض OpenPatent المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `OpenPatent.json`:
...
**للوكلاء المدمجين** (في `OpenPatent.json`):
