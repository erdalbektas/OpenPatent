...
أنشئ الملف `.OpenPatent/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenPatent أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenPatent:
...
- على مستوى النظام: `~/.config/OpenPatent/commands/`
- لكل مشروع: `.OpenPatent/commands/`
...
يتضمن OpenPatent عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
