...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات OpenPatent. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في OpenPatent تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات OpenPatent.
...
يأتي OpenPatent مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.OpenPatent/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `OpenPatent.json` لديك:
...
- عالمي: `~/.config/OpenPatent/modes/`
- للمشروع: `.OpenPatent/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم OpenPatent القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات OpenPatent العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.OpenPatent/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/OpenPatent/modes/` لأوضاع عالمية:
