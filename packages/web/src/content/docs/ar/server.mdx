---
title: الخادم
description: تفاعل مع خادم OpenPatent عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `OpenPatent serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل OpenPatent استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `OpenPatent.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `OpenPatent`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `OpenPatent serve` و `OpenPatent web`.
...
عند تشغيل `OpenPatent` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم OpenPatent للتفاعل مع OpenPatent برمجيا.
:::

تتيح هذه البنية لـ OpenPatent دعم عدة عملاء وتمكّنك من التفاعل مع OpenPatent برمجيا.

يمكنك تشغيل `OpenPatent serve` لبدء خادم مستقل. إذا كانت واجهة OpenPatent في terminal (TUI)
قيد التشغيل، فسيبدأ `OpenPatent serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenPatent لـ [IDE](/docs/ide).
...
يعرض خادم OpenPatent واجهات API التالية.
