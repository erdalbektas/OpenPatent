---
title: Eklentiler
description: OpenPatent'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenPatent'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenPatent'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.OpenPatent/plugins/` - Proje düzeyinde eklentiler
- `~/.config/OpenPatent/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/OpenPatent/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/OpenPatent/OpenPatent.json`)
2. Project config (`OpenPatent.json`)
3. Global eklenti dizini (`~/.config/OpenPatent/plugins/`)
4. Proje eklenti dizini (`.OpenPatent/plugins/`)
...
OpenPatent bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için OpenPatent'lu bir SDK istemcisi.
...
OpenPatent'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenPatent masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenPatent'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenPatent'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenPatent'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://OpenPatent.ai/docs/sdk)'e bakın.
