...
OpenPatent ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak OpenPatent kendi `OpenPatent` temasını kullanır.
...
OpenPatent birden fazla yerleşik temayla gelir.
...
- OpenPatent'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
OpenPatent, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/OpenPatent/themes/*.json` veya `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.OpenPatent/themes/*.json`
4. **Geçerli çalışma dizini** - `./.OpenPatent/themes/*.json`
