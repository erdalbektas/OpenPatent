---
title: CLI
description: OpenPatent CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenPatent CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
OpenPatent
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, OpenPatent ile programlı olarak etkileşim kurmanıza olanak tanır.
...
OpenPatent terminal kullanıcı arayüzünü başlatır.
...
OpenPatent CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

OpenPatent için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir OpenPatent arka uç sunucusuna bir terminal ekleyin.

```bash
OpenPatent attach [url]
```

Bu, TUI öğesinin uzak bir OpenPatent arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
OpenPatent, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `OpenPatent auth login` kullanabilirsiniz. Bu `~/.local/share/OpenPatent/auth.json`'de saklanıyor.

```bash
OpenPatent auth login
```

OpenPatent başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları OpenPatent'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek OpenPatent'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `OpenPatent serve` örneğine de ekleyebilirsiniz:
...
| `--attach`   |      | Çalışan bir OpenPatent sunucusuna ekleyin (ör. http://localhost:4096)               |
...
API erişimi için headless bir OpenPatent sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
OpenPatent serve
```

Bu, TUI arayüzü olmadan OpenPatent işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `OpenPatent` şeklindedir).
...
OpenPatent oturumlarını yönetin.
...
Tüm OpenPatent oturumlarını listeleyin.
...
OpenPatent oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya OpenPatent paylaşımından URL oturum verilerini içe aktarın.

```bash
OpenPatent import <file>
```

Yerel bir dosyadan veya OpenPatent paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir OpenPatent sunucusu başlatın.

```bash
OpenPatent web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla OpenPatent'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `OpenPatent` şeklindedir).
...
OpenPatent'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
OpenPatent'u en son sürüme veya belirli bir sürüme günceller.
...
OpenPatent CLI aşağıdaki global bayrakları destekler.
...
OpenPatent ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `OpenPatent`) |
