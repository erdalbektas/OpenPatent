...
_Model Bağlam Protokolü_ veya MCP kullanarak OpenPatent'a harici araçlar ekleyebilirsiniz. OpenPatent hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [OpenPatent Config](https://OpenPatent.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/OpenPatent` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
OpenPatent, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde OpenPatent şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa OpenPatent, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `OpenPatent mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından OpenPatent, belirteçleri `~/.local/share/OpenPatent/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra OpenPatent'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi OpenPatent yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve OpenPatent'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
