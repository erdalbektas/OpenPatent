---
title: ACP Desteği
description: OpenPatent'u ACP uyumlu herhangi bir editörde kullanın.
---

OpenPatent, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
OpenPatent'u ACP aracılığıyla kullanmak için düzenleyicinizi `OpenPatent acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, OpenPatent'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'OpenPatent' ajanını kullanın.
...
OpenPatent'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı OpenPatent'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
OpenPatent, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- OpenPatent yapılandırmanızda yapılandırılmış MCP sunucuları
