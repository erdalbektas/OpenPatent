...
:::caution
Modlar artık OpenPatent yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

OpenPatent'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya OpenPatent yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
OpenPatent iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.OpenPatent/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`OpenPatent.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, OpenPatent modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global OpenPatent yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.OpenPatent/modes/` veya genel modlar için `~/.config/OpenPatent/modes/`'de mod dosyaları oluşturun:
