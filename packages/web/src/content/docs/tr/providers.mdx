---
title: Sağlayıcılar
description: OpenPatent'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenPatent, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı OpenPatent yapılandırmanızda yapılandırın.
...
`~/.local/share/OpenPatent/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları OpenPatent'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenPatent Zen, OpenPatent ekibi tarafından sağlanan modellerin bir listesidir.
OpenPatent ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenPatent Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, OpenPatent'u seçin ve [OpenPatent.ai/auth](https://OpenPatent.ai/auth)'ye gidin.
...
OpenPatent'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u OpenPatent ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   OpenPatent'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `OpenPatent.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi OpenPatent'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   OpenPatent'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in OpenPatent auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
OpenPatent, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. OpenPatent'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`OpenPatent.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [OpenPatent-gitlab-auth](https://www.npmjs.com/package/@gitlab/OpenPatent-gitlab-auth) ana sayfasında.

##### Yapılandırma

`OpenPatent.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi OpenPatent'la kullanmak için:
...
Google Vertex AI'yi OpenPatent ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları OpenPatent çalıştırılırken ayarlayın.
...
Helicone'un OpenPatent aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her OpenPatent görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [OpenPatent-helicone-session](https://github.com/H2Shami/OpenPatent-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her OpenPatent görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
OpenPatent'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla OpenPatent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla OpenPatent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini OpenPatent için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/OpenPatent)'a bakın.
:::
...
Ollama Cloud'u OpenPatent ile kullanmak için:
...
3. OpenPatent'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: OpenPatent'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenPatent Zen, OpenPatent ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenPatent Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenPatent Zen**'i arayın.

   ```txt
   /connect
   ```

3. OpenPatent API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   OpenPatent yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca OpenPatent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri OpenPatent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı OpenPatent ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `OpenPatent.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları OpenPatent'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `OpenPatent auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için OpenPatent yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, OpenPatent yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
