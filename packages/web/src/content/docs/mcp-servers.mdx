...
You can add external tools to OpenPatent using the _Model Context Protocol_, or MCP. OpenPatent supports both local and remote servers.
...
You can define MCP servers in your [OpenPatent Config](https://OpenPatent.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/OpenPatent` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenPatent automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenPatent will:
...
If the server requires authentication, OpenPatent will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `OpenPatent mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenPatent will store the tokens securely in `~/.local/share/OpenPatent/mcp-auth.json`.
...
Your MCPs are available as tools in OpenPatent, alongside built-in tools. So you can manage them through the OpenPatent config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenPatent to your Sentry account.
