---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenPatent.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenPatent bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenPatent-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/OpenPatent/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenPatent-konfigurationen.
...
OpenPatent Zen er en liste over modeller leveret af OpenPatent-teamet, som er blevet testet og verificeret til at fungere godt med OpenPatent. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenPatent Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg OpenPatent og gå til [OpenPatent.ai/auth](https://OpenPatent.ai/auth).
...
Det fungerer som alle andre udbydere i OpenPatent og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med OpenPatent:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører OpenPatent:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `OpenPatent.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenPatent understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at OpenPatent skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem OpenPatent-konfigurationen.
...
gemme tokenet i OpenPatent auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenPatent bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenPatent
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`OpenPatent.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [OpenPatent-gitlab-auth](https://www.npmjs.com/package/@gitlab/OpenPatent-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `OpenPatent.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med OpenPatent:
...
Sådan bruger du Google Vertex AI med OpenPatent:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører OpenPatent.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem OpenPatent, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [OpenPatent-helicone-session](https://github.com/H2Shami/OpenPatent-helicone-session) plugin for automatisk at logge hver OpenPatent-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenPatent-samtale opført som en separat session.
...
Du kan konfigurere OpenPatent til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere OpenPatent til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere OpenPatent til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenPatent. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/OpenPatent) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenPatent:
...
3. Kopier API-nøglen til brug i OpenPatent.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenPatent, skal du hente modelinformationen lokalt:
...
OpenPatent Zen er en liste over testede og verificerede modeller leveret af OpenPatent-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenPatent Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenPatent Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenPatent API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem OpenPatent-konfigurationen.
...
5. Du kan også tilpasse dem gennem OpenPatent-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenPatent:
...
Du kan også tilpasse modeller gennem OpenPatent-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med OpenPatent. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `OpenPatent.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenPatent forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `OpenPatent auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek OpenPatent-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i OpenPatent-konfigurationen.
