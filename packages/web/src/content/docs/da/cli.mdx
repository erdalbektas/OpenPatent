---
title: CLI
description: OpenPatent CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenPatent CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
OpenPatent
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med OpenPatent programmatisk.
...
Start OpenPatent terminalbrugergrænsefladen.
...
OpenPatent CLI har også følgende kommandoer.

---

### agent

Administrer agenter i OpenPatent.
...
Tilslut en terminal til en allerede kørende OpenPatent backend-server startet via `serve` eller `web` kommandoer.

```bash
OpenPatent attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern OpenPatent backend. For eksempel:
...
OpenPatent drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `OpenPatent auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

Når OpenPatent starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i OpenPatent.
...
Kør OpenPatent i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `OpenPatent serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| `--attach`   |      | Tilslut til en kørende OpenPatent-server (f.eks. http://localhost:4096)               |
...
Start en hovedløs OpenPatent-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
OpenPatent serve
```

Dette starter en HTTP-server, der giver API-adgang til OpenPatent-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `OpenPatent`).
...
Administrator OpenPatent sessionsholder.
...
Liste over alle OpenPatent sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine OpenPatent-sessioner.
...
Importer sessionsdata fra en JSON fil eller OpenPatent del URL.

```bash
OpenPatent import <file>
```

Du kan importere fra en lokal fil eller en OpenPatent share URL.
...
Start en hovedløs OpenPatent-server med en webgrænseflade.

```bash
OpenPatent web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenPatent via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `OpenPatent`).
...
Afinstaller OpenPatent og fjern alle relaterede filer.
...
Opdaterer OpenPatent til den nyeste version eller en specifik version.
...
OpenPatent CLI tager følgende globale flag.
...
OpenPatent kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `OpenPatent`) |
