...
Med OpenPatent kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenPatent vårt eget `OpenPatent`-tema.
...
OpenPatent støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/OpenPatent/themes/*.json` eller `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.OpenPatent/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.OpenPatent/themes/*.json`
