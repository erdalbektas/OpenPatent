...
For at fejlfinde problemer med OpenPatent, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\OpenPatent\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `OpenPatent --log-level DEBUG`.

---

## Lagring

OpenPatent gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/OpenPatent/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\OpenPatent`
...
OpenPatent Desktop kører en lokal OpenPatent-server (`OpenPatent-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenPatent`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/OpenPatent/OpenPatent.jsonc` (eller `~/.config/OpenPatent/OpenPatent.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/OpenPatent/OpenPatent.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\OpenPatent\OpenPatent.jsonc`
...
OpenPatent kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/OpenPatent/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\OpenPatent\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenPatent kan genopbygge den.

1. Afslut OpenPatent Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/OpenPatent`
- **Linux**: slet `~/.cache/OpenPatent` (eller kør `rm -rf ~/.cache/OpenPatent`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\OpenPatent`

3. Start OpenPatent Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenPatent Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `OpenPatent.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver OpenPatent Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenPatent.

---

### Meddelelser vises ikke

OpenPatent Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenPatent i dine OS-indstillinger, og
...
1. Afslut OpenPatent Desktop.
2. Find og slet disse filer (de findes i OpenPatent Desktop-appens datamappe):

- `OpenPatent.settings.dat` (skrivebordsstandardserver URL)
- `OpenPatent.global.dat` og `OpenPatent.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OpenPatent:
...
For at finde ud af hvilke modeller du har adgang til, kør `OpenPatent models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\OpenPatent`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. OpenPatent installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\OpenPatent`

2. Start OpenPatent på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge OpenPatent til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
OpenPatent vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
