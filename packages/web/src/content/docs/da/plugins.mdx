---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenPatent.
---

Plugins giver dig mulighed for at udvide OpenPatent ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenPatents standardadfærd.
...
- `.OpenPatent/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/OpenPatent/node_modules/`.
...
1. Global konfiguration (`~/.config/OpenPatent/OpenPatent.json`)
2. Projektkonfiguration (`OpenPatent.json`)
3. Global plugin-mappe (`~/.config/OpenPatent/plugins/`)
4. Projekt plugin bibliotek (`.OpenPatent/plugins/`)
...
OpenPatent kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En OpenPatent SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide OpenPatent.
...
:::note
Hvis du bruger OpenPatent desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå OpenPatent i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til OpenPatent:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som OpenPatent kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for OpenPatent sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://OpenPatent.ai/docs/sdk) for detaljer.
