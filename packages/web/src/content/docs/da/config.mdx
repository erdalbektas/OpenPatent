---
title: Konfiguration
description: Ved at bruge OpenPatent JSON konfig.
---

Du kan konfigurere OpenPatent ved hjælp af en JSON-konfigurationsfil.

---

## Format

OpenPatent understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis dine globale konfigurationssæt `theme: "OpenPatent"` og `autoupdate: true`, og dine projektkonfigurationssæt `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfiguration integrere alle tre indstillinger.
...
1. **Fjernkonfiguration** (fra `.well-known/OpenPatent`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/OpenPatent/OpenPatent.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`OpenPatent.json` i projekt) - projektspecifikke indstillinger
5. **`.OpenPatent` mapper** - agent, kommandoer, plugins
...
:::note
`.OpenPatent` og `~/.config/OpenPatent` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/OpenPatent`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OpenPatent-konfiguration i `~/.config/OpenPatent/OpenPatent.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `OpenPatent.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når OpenPatent starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
modes og plugins ligesom standard `.OpenPatent` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.OpenPatent` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`OpenPatent.ai/config.json`**](https://OpenPatent.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `OpenPatent serve` og `OpenPatent web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din OpenPatent-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `OpenPatent.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din OpenPatent-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger OpenPatent at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din OpenPatent-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/OpenPatent/agents/` eller `.OpenPatent/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil OpenPatent falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`OpenPatent run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/OpenPatent/commands/` eller `.OpenPatent/commands/`. [Learn more here](/docs/commands).
...
OpenPatent vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard OpenPatent **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide OpenPatent med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.OpenPatent/plugins/` eller `~/.config/OpenPatent/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse OpenPatent til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
