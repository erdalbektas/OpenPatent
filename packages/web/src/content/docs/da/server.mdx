---
title: Server
description: Interager med OpenPatent-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `OpenPatent serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en OpenPatent-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `OpenPatent.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `OpenPatent`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `OpenPatent serve` og `OpenPatent web`.
...
Når du kører `OpenPatent` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug OpenPatent-serveren til at interagere med OpenPatent programmatisk.
:::

Denne arkitektur lader OpenPatent understøtte flere klienter og lader dig interagere med OpenPatent programmatisk.

Du kan køre `OpenPatent serve` for at starte en selvstændig server. Hvis du har
OpenPatent TUI kørende, vil `OpenPatent serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenPatent [IDE](/docs/ide) plugins.
...
OpenPatent-serveren viser følgende API'er.
