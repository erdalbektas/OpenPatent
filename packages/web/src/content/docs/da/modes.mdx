...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i OpenPatent-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i OpenPatent giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem OpenPatent-konfigurationen.
...
OpenPatent leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.OpenPatent/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `OpenPatent.json`-konfigurationsfil:
...
- Globalt: `~/.config/OpenPatent/modes/`
...
Hvis der ikke er angivet nogen temperatur, bruger OpenPatent modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale OpenPatent-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.OpenPatent/modes/` for projektspecifikke tilstande eller `~/.config/OpenPatent/modes/` for globale tilstande:
