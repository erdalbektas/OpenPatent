---
title: CLI
description: OpenPatent CLI 选项和命令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenPatent CLI 在不带任何参数运行时，默认启动 [TUI](/docs/tui)。

```bash
OpenPatent
```

但它也接受本页面中记录的命令，使您可以通过编程方式与 OpenPatent 进行交互。
...
将终端连接到已通过 `serve` 或 `web` 命令启动的 OpenPatent 后端服务器。

```bash
OpenPatent attach [url]
```

这允许将 TUI 与远程 OpenPatent 后端配合使用。例如：
...
OpenPatent 基于 [Models.dev](https://models.dev) 的提供商列表运行，因此您可以使用 `OpenPatent auth login` 为任何想要使用的提供商配置 API 密钥。密钥存储在 `~/.local/share/OpenPatent/auth.json` 中。

```bash
OpenPatent auth login
```

OpenPatent 启动时会从凭据文件加载提供商信息，同时也会加载环境变量或项目中 `.env` 文件中定义的密钥。
...
使用 `--refresh` 标志可以更新缓存的模型列表。当提供商新增了模型并且您希望在 OpenPatent 中看到它们时，此功能非常有用。
...
您还可以连接到正在运行的 `OpenPatent serve` 实例，以避免每次运行时 MCP 服务器的冷启动时间：
...
| `--attach`   |      | 连接到正在运行的 OpenPatent 服务器（例如 http://localhost:4096） |
...
启动无界面的 OpenPatent 服务器以提供 API 访问。查看[服务器文档](/docs/server)了解完整的 HTTP 接口。

```bash
OpenPatent serve
```

此命令启动一个 HTTP 服务器，提供对 OpenPatent 功能的 API 访问，无需 TUI 界面。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `OpenPatent`）。
...
显示 OpenPatent 会话的 Token 用量和费用统计信息。
...
从 JSON 文件或 OpenPatent 分享链接导入会话数据。

```bash
OpenPatent import <file>
```

您可以从本地文件或 OpenPatent 分享链接导入。
...
启动带有 Web 界面的无界面 OpenPatent 服务器。

```bash
OpenPatent web
```

此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 OpenPatent。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `OpenPatent`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆盖基本认证用户名（默认为 `OpenPatent`） |
