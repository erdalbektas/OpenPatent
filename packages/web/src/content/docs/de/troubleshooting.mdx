...
Wenn OpenPatent Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `OpenPatent --log-level DEBUG`.

---

## Speicher

OpenPatent speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
OpenPatent Desktop startet im Hintergrund einen lokalen OpenPatent-Server (`OpenPatent-cli`-Sidecar).
...
- Nur macOS: `OpenPatent`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit OpenPatent hast:
