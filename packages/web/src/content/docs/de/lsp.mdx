---
title: LSP Server
description: OpenPatent lässt sich in Ihren LSP-Server integrieren.
---

OpenPatent lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

OpenPatent verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
Wenn OpenPatent eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer OpenPatent-Konfiguration anpassen.
...
```json title="OpenPatent.json" {5-7}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="OpenPatent.json" {5-9}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "lsp": {
    "typescript": {
