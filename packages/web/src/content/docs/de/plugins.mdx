---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenPatent zu erweitern.
---

Mit Plugins können Sie OpenPatent erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenPatent zu ändern.
...
- `.OpenPatent/plugins/` – Plugins auf Projektebene
- `~/.config/OpenPatent/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/OpenPatent/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/OpenPatent/OpenPatent.json`)
2. Projektkonfiguration (`OpenPatent.json`)
3. Globales Plugin-Verzeichnis (`~/.config/OpenPatent/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.OpenPatent/plugins/`)
...
OpenPatent führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenPatent-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenPatent erweitern können.
...
:::note
Wenn Sie die OpenPatent-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenPatent `.env`-Dateien liest:
...
Plugins können OpenPatent auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenPatent aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenPatent zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://OpenPatent.ai/docs/sdk).
