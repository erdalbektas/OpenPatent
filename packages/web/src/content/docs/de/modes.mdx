...
:::caution
Modi werden jetzt über die Option `agent` in der OpenPatent-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in OpenPatent können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die OpenPatent-Konfiguration.
...
OpenPatent verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.OpenPatent/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `OpenPatent.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet OpenPatent modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale OpenPatent-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.OpenPatent/modes/` für projektspezifische Modi oder `~/.config/OpenPatent/modes/` für globale Modi:
