---
title: Regeln
description: Lege eigene Anweisungen fuer OpenPatent fest.
---

Du kannst OpenPatent ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in OpenPatent den Befehl `/init` aus.
...
So kann OpenPatent sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht OpenPatent sie zu erweitern.
...
OpenPatent kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/OpenPatent/AGENTS.md` speichern.
Diese gelten in allen OpenPatent-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenPatent dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/OpenPatent/AGENTS.md` existiert)
...
Beim Start sucht OpenPatent in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/OpenPatent/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `OpenPatent.json` oder in `~/.config/OpenPatent/OpenPatent.json` angeben.
...
OpenPatent parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit OpenPatent.json

Empfohlen ist das `instructions`-Feld in der `OpenPatent.json`:
...
Du kannst OpenPatent explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `OpenPatent.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
