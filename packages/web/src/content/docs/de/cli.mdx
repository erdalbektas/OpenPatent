---
title: CLI
description: OpenPatent CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der OpenPatent CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
OpenPatent
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit OpenPatent interagieren.
...
Starten Sie die Terminalbenutzeroberfläche OpenPatent.
...
Der OpenPatent CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für OpenPatent verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden OpenPatent-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
OpenPatent attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend OpenPatent. Zum Beispiel:
...
OpenPatent wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `OpenPatent auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/OpenPatent/auth.json` gespeichert.

```bash
OpenPatent auth login
```

Wenn OpenPatent startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in OpenPatent sehen möchten.
...
Führen Sie OpenPatent im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `OpenPatent serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| `--attach`   |      | An einen laufenden OpenPatent-Server anschließen (z.B. http://localhost:4096)                         |
...
Starten Sie einen Headless-OpenPatent-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
OpenPatent serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf OpenPatent-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `OpenPatent`).
...
OpenPatent-Sitzungen verwalten.
...
Listen Sie alle OpenPatent-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre OpenPatent-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer OpenPatent-Freigabe-URL.

```bash
OpenPatent import <file>
```

Sie können aus einer lokalen Datei oder einer OpenPatent-Freigabe-URL importieren.
...
Starten Sie einen Headless-OpenPatent-Server mit einer Weboberfläche.

```bash
OpenPatent web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf OpenPatent zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `OpenPatent`).
...
Deinstallieren Sie OpenPatent und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert OpenPatent auf die neueste Version oder eine bestimmte Version.
...
Der OpenPatent CLI akzeptiert die folgenden globalen Flags.
...
OpenPatent kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `OpenPatent`) |
