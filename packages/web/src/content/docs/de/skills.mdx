...
Agent Skills erlauben OpenPatent, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenPatent sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenPatent vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.OpenPatent/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/OpenPatent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.OpenPatent/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenPatent listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `OpenPatent.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `OpenPatent.json`):
