---
title: Server
description: Nutze den OpenPatent-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `OpenPatent serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein OpenPatent-Client nutzen kann.
...
Der Benutzername ist standardmaessig `OpenPatent`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `OpenPatent serve` und `OpenPatent web`.
...
Wenn du `OpenPatent` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den OpenPatent-Server, um OpenPatent programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `OpenPatent serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `OpenPatent serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenPatent-[IDE](/docs/ide)-Plugins.
...
Der OpenPatent-Server stellt folgende APIs bereit.
