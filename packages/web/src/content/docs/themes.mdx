...
With OpenPatent you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenPatent uses our own `OpenPatent` theme.
...
OpenPatent comes with several built-in themes.
...
OpenPatent supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/OpenPatent/themes/*.json` or `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.OpenPatent/themes/*.json`
4. **Current working directory** - Defined in `./.OpenPatent/themes/*.json`
