...
Puoi aggiungere strumenti esterni a OpenPatent usando il _Model Context Protocol_, o MCP. OpenPatent supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenPatent Config](https://OpenPatent.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/OpenPatent`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
OpenPatent gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenPatent:
...
Se il server richiede autenticazione, OpenPatent ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `OpenPatent mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenPatent memorizzera' i token in modo sicuro in `~/.local/share/OpenPatent/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenPatent insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenPatent come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenPatent al tuo account Sentry.
