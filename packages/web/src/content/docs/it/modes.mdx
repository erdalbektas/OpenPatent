...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di OpenPatent. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in OpenPatent ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di OpenPatent.
...
OpenPatent include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.OpenPatent/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `OpenPatent.json`:
...
- Globale: `~/.config/OpenPatent/modes/`
- Progetto: `.OpenPatent/modes/`
...
Se non viene specificata alcuna temperatura, OpenPatent usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di OpenPatent sia per quella specifica del progetto.
...
Crea file di modalita in `.OpenPatent/modes/` per modalita specifiche del progetto oppure in `~/.config/OpenPatent/modes/` per modalita globali:
