...
In OpenPatent esistono due tipi di agenti: agenti primari e subagenti.
...
OpenPatent include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenPatent include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

OpenPatent include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `OpenPatent.json`:
...
- Globale: `~/.config/OpenPatent/agents/`
- Per progetto: `.OpenPatent/agents/`
...
Se non specifichi una temperature, OpenPatent usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenPatent sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenPatent usa il formato `provider/model-id`. Per esempio, se stai usando [OpenPatent Zen](/docs/zen), useresti `OpenPatent/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `OpenPatent models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/OpenPatent).
:::
