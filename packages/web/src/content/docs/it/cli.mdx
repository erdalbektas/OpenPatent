---
title: CLI
description: Opzioni e comandi della CLI di OpenPatent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di OpenPatent, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
OpenPatent
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con OpenPatent in modo programmatico.
...
Avvia l'interfaccia testuale di OpenPatent (interfaccia utente da terminale).
...
La CLI di OpenPatent include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per OpenPatent.
...
Collega un terminale a un backend server di OpenPatent già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
OpenPatent attach [url]
```

Questo consente di usare la TUI con un backend OpenPatent remoto. Per esempio:
...
OpenPatent si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `OpenPatent auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

Quando OpenPatent si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in OpenPatent.
...
Esegui OpenPatent in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `OpenPatent serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| `--attach`   |       | Attach a un server OpenPatent in esecuzione (es. http://localhost:4096)    |
...
Avvia un server OpenPatent headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
OpenPatent serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di OpenPatent senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `OpenPatent`).
...
Gestisci le sessioni OpenPatent.
...
Elenca tutte le sessioni OpenPatent.
...
Mostra statistiche di utilizzo token e costo per le sessioni OpenPatent.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione OpenPatent.

```bash
OpenPatent import <file>
```

Puoi importare da un file locale o da un URL di condivisione OpenPatent.
...
Avvia un server OpenPatent headless con interfaccia web.

```bash
OpenPatent web
```

Avvia un server HTTP e apre un browser per accedere a OpenPatent tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `OpenPatent`).
...
Disinstalla OpenPatent e rimuove tutti i file correlati.
...
Aggiorna OpenPatent all'ultima versione o a una versione specifica.
...
La CLI di OpenPatent accetta i seguenti flag globali.
...
OpenPatent può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `OpenPatent`)     |
