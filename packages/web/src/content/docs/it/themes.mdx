...
Con OpenPatent puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OpenPatent usa il tema `OpenPatent`.
...
OpenPatent include diversi temi integrati.
...
- Vuole che OpenPatent corrisponda all'aspetto del terminale
...
OpenPatent supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/OpenPatent/themes/*.json` o `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.OpenPatent/themes/*.json`
4. **Directory di lavoro corrente** - in `./.OpenPatent/themes/*.json`
