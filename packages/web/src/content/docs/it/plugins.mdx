---
title: Plugin
description: Scrivi plugin per estendere OpenPatent.
---

I plugin ti permettono di estendere OpenPatent agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di OpenPatent.
...
- `.OpenPatent/plugins/` - plugin a livello progetto
- `~/.config/OpenPatent/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/OpenPatent/node_modules/`.
...
1. Config globale (`~/.config/OpenPatent/OpenPatent.json`)
2. Config di progetto (`OpenPatent.json`)
3. Directory plugin globale (`~/.config/OpenPatent/plugins/`)
4. Directory plugin di progetto (`.OpenPatent/plugins/`)
...
OpenPatent esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di OpenPatent per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere OpenPatent.
...
:::note
Se usi l'app desktop di OpenPatent, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a OpenPatent di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a OpenPatent:
...
L'helper `tool` crea uno strumento personalizzato che OpenPatent puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in OpenPatent insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://OpenPatent.ai/docs/sdk) per i dettagli.
