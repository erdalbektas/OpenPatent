---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenPatent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenPatent fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenPatent avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenPatent, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenPatent. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenPatent. _Alias_: `/quit`, `/q`
...
Puoi personalizzare il comportamento della TUI tramite il file di config di OpenPatent.
