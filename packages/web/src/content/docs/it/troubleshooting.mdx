...
Per diagnosticare problemi con OpenPatent, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\OpenPatent\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `OpenPatent --log-level DEBUG`.

---

## Archiviazione

OpenPatent salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/OpenPatent/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\OpenPatent`
...
OpenPatent Desktop esegue in background un server locale di OpenPatent (il sidecar `OpenPatent-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `OpenPatent` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux**: `~/.config/OpenPatent/OpenPatent.jsonc` (or `~/.config/OpenPatent/OpenPatent.json`)
- **macOS/Linux** (older installs): `~/.local/share/OpenPatent/OpenPatent.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\OpenPatent\OpenPatent.jsonc`
...
OpenPatent puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/OpenPatent/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\OpenPatent\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che OpenPatent possa ricostruirla.

1. Quit OpenPatent Desktop completely.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/OpenPatent`
- **Linux**: elimina `~/.cache/OpenPatent` (oppure esegui `rm -rf ~/.cache/OpenPatent`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\OpenPatent`

3. Restart OpenPatent Desktop.

---

### Risolvi problemi di connessione al server

OpenPatent Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `OpenPatent.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, OpenPatent Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di OpenPatent.

---

### Notifiche non visualizzate

OpenPatent Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per OpenPatent nelle impostazioni del sistema operativo, e
...
1. Quit OpenPatent Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app OpenPatent Desktop):

- `OpenPatent.settings.dat` (desktop default server URL)
- `OpenPatent.global.dat` e `OpenPatent.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con OpenPatent:
...
Per capire a quali modelli hai accesso, esegui `OpenPatent models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\OpenPatent`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. OpenPatent installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\OpenPatent`

2. Riavvia OpenPatent per reinstallare i pacchetti provider piu recenti

Questo forzera OpenPatent a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
OpenPatent rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
