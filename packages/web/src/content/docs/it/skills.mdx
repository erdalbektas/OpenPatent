...
Le skill degli agenti permettono a OpenPatent di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenPatent cerca in queste posizioni:

- Config di progetto: `.OpenPatent/skills/<name>/SKILL.md`
- Config globale: `~/.config/OpenPatent/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenPatent risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.OpenPatent/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/OpenPatent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.OpenPatent/skills/git-release/SKILL.md` cosi':
...
OpenPatent elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `OpenPatent.json`:
...
**Per agenti integrati** (in `OpenPatent.json`):
