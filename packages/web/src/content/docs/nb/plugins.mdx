...
description: Skriv dine egne programtillegg for å utvide OpenPatent.
---

Plugins lar deg utvide OpenPatent ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenPatent.
...
- `.OpenPatent/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/OpenPatent/node_modules/`.
...
1. Global konfigurasjon (`~/.config/OpenPatent/OpenPatent.json`)
2. Prosjektkonfigurasjon (`OpenPatent.json`)
3. Global plugin-katalog (`~/.config/OpenPatent/plugins/`)
4. Prosjektpluginkatalog (`.OpenPatent/plugins/`)
...
OpenPatent kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenPatent SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenPatent.
...
:::note
Hvis du bruker OpenPatent-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenPatent fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenPatent:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenPatent kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenPatent sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://OpenPatent.ai/docs/sdk) for detaljer.
