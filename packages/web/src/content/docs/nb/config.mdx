---
title: Konfigurasjon
description: Ved å bruke OpenPatent JSON-konfigurasjonen.
---

Du kan konfigurere OpenPatent ved å bruke en JSON konfigurasjonsfil.

---

## Format

OpenPatent støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "OpenPatent"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/OpenPatent`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/OpenPatent/OpenPatent.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`OpenPatent.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.OpenPatent` kataloger** - agenter, kommandoer, plugins
...
:::note
`.OpenPatent` og `~/.config/OpenPatent` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/OpenPatent`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale OpenPatent-konfigurasjon i `~/.config/OpenPatent/OpenPatent.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `OpenPatent.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når OpenPatent starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
moduser og plugins akkurat som standard `.OpenPatent` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.OpenPatent`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`OpenPatent.ai/config.json`**](https://OpenPatent.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `OpenPatent serve` og `OpenPatent web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage OpenPatent-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `OpenPatent.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i OpenPatent-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver OpenPatent å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan konfigurere temaet du vil bruke i OpenPatent-konfigurasjonen gjennom alternativet `theme`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/OpenPatent/agents/` eller `.OpenPatent/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil OpenPatent falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`OpenPatent run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/OpenPatent/commands/` eller `.OpenPatent/commands/`. [Les mer her](/docs/commands).
...
OpenPatent vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater OpenPatent alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider OpenPatent med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.OpenPatent/plugins/` eller `~/.config/OpenPatent/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense OpenPatent til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
