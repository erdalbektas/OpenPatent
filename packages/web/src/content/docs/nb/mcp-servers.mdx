...
Du kan legge til eksterne verktøy i OpenPatent ved å bruke _Model Context Protocol_ eller MCP. OpenPatent støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenPatent Config](https://OpenPatent.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="OpenPatent.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/OpenPatent`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="OpenPatent.jsonc" {15}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenPatent håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenPatent:
...
Hvis serveren krever autentisering, vil OpenPatent be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `OpenPatent mcp auth <server-name>`.
...
```json title="OpenPatent.json" {7-11}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
OpenPatent mcp auth my-oauth-server
```
...
```bash
OpenPatent mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenPatent lagre tokens sikkert i `~/.local/share/OpenPatent/mcp-auth.json`.
...
OpenPatent mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenPatent, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenPatent-konfigurasjonen som alle andre verktøy.
...
```json title="OpenPatent.json" {11, 14-18}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="OpenPatent.json" {4-8}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
OpenPatent mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenPatent til Sentry-kontoen din.
...
```json title="OpenPatent.json" {4-7}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="OpenPatent.json" {7-9}
{
  "$schema": "https://OpenPatent.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenPatent-samtale oppført som en separat økt.
