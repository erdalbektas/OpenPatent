---
title: CLI
description: OpenPatent CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenPatent CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
OpenPatent
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med OpenPatent programmatisk.
...
Start OpenPatent-terminalbrukergrensesnittet.
...
Administrer agenter for OpenPatent.
...
Koble en terminal til en allerede kjørende OpenPatent-backend-server startet via `serve` eller `web` kommandoer.

```bash
OpenPatent attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern OpenPatent-backend. For eksempel:
...
OpenPatent drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `OpenPatent auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

Når OpenPatent starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i OpenPatent.
...
Kjør OpenPatent i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `OpenPatent serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| `--attach`   |      | Koble til en kjørende OpenPatent-server (f.eks. http://localhost:4096)     |
...
Start en headless OpenPatent-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
OpenPatent serve
```

Dette starter en HTTP-server som gir API tilgang til OpenPatent-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `OpenPatent`).
...
List opp alle OpenPatent-økter.
...
Vis tokenbruk og kostnadsstatistikk for OpenPatent-øktene dine.
...
Importer øktdata fra en JSON-fil eller en OpenPatent share-URL.
...
Start en headless OpenPatent-server med et webgrensesnitt.

```bash
OpenPatent web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til OpenPatent via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `OpenPatent`).
...
Avinstaller OpenPatent og fjern alle relaterte filer.
...
Oppdaterer OpenPatent til siste versjon eller en spesifikk versjon.
...
OpenPatent CLI bruker følgende globale flagg.
...
OpenPatent kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `OpenPatent`) |
