---
title: Tjener
description: Samhandle med OpenPatent-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `OpenPatent serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenPatent-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `OpenPatent.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `OpenPatent` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `OpenPatent serve` og `OpenPatent web`.
...
Når du kjører `OpenPatent` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenPatent-serveren til å samhandle med OpenPatent programmatisk.
:::

Denne arkitekturen lar OpenPatent støtte flere klienter og lar deg samhandle med OpenPatent programmatisk.

Du kan kjøre `OpenPatent serve` for å starte en frittstående server. Hvis du har
OpenPatent TUI kjører, vil `OpenPatent serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenPatent [IDE](/docs/ide) plugins.
...
OpenPatent-serveren viser følgende APIer.
