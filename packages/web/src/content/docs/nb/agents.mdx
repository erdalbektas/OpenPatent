...
Det er to typer agenter i OpenPatent; primære agenter og underagenter.
...
OpenPatent kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenPatent kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenPatent kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `OpenPatent.json`:
...
- Globalt: `~/.config/OpenPatent/agents/`
- Per prosjekt: `.OpenPatent/agents/`
...
Hvis ingen temperatur er spesifisert, bruker OpenPatent modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenPatent-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenPatent-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenPatent Zen](/docs/zen), vil du bruke `OpenPatent/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `OpenPatent models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/OpenPatent).
:::
