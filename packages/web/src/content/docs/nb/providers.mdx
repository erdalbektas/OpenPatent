---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenPatent.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenPatent bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenPatent-konfigurasjonen.
...
i `~/.local/share/OpenPatent/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenPatent
konfig.
...
OpenPatent Zen er en liste over modeller levert av OpenPatent-teamet som har vært
testet og verifisert for å fungere godt med OpenPatent. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenPatent Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg OpenPatent og gå til [OpenPatent.ai/auth](https://OpenPatent.ai/auth).
...
Det fungerer som alle andre leverandører i OpenPatent og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med OpenPatent:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenPatent:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `OpenPatent.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i OpenPatent støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenPatent skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenPatent-konfigurasjonen.
...
for å lagre token i OpenPatent auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenPatent bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenPatent
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`OpenPatent.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [OpenPatent-gitlab-auth](https://www.npmjs.com/package/@gitlab/OpenPatent-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `OpenPatent.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenPatent:
...
Slik bruker du Google Vertex AI med OpenPatent:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenPatent.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenPatent, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [OpenPatent-helicone-session](https://github.com/H2Shami/OpenPatent-helicone-session) plugin for automatisk å logge hver OpenPatent-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenPatent-samtale oppført som en separat økt.
...
Du kan konfigurere OpenPatent for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenPatent for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenPatent for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenPatent. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/OpenPatent) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenPatent:
...
3. Kopier API-nøkkelen for bruk i OpenPatent.
...
6. **Viktig**: Før du bruker skymodeller i OpenPatent, må du hente modellinformasjonen lokalt:
...
OpenPatent Zen er en liste over testede og verifiserte modeller levert av OpenPatent-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenPatent Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenPatent Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenPatent API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenPatent-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenPatent-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenPatent:
...
Du kan også tilpasse modeller gjennom OpenPatent-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenPatent. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `OpenPatent.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenPatent forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `OpenPatent auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenPatent-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenPatent-konfigurasjonen.
