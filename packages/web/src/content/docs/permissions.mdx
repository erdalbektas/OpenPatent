...
By default, openpatent allows most operations without approval, except `doom_loop` and `external_directory` which default to `ask`. You can configure this using the `permission` option.
...
```json title="openpatent.json" {3-7,10-14}
{
  "$schema": "https://openpatent.ai/config.json",
  "permission": {
    "bash": {
