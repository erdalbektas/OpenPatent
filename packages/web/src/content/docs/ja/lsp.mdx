---
title: LSPサーバー
description: OpenPatent は LSP サーバーと統合します。
---

OpenPatent は言語サーバープロトコル (LSP) と統合して、LLM がコードベースと対話できるようにします。診断を使用して LLM にフィードバックを提供します。

---

## 組み込み

OpenPatent には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
OpenPatent がファイルを開くと、次のようになります。
...
OpenPatent 設定の `lsp` セクションを通じて LSP サーバーをカスタマイズできます。
