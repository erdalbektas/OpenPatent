---
title: CLI
description: OpenPatent CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenPatent CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
OpenPatent
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、OpenPatent をプログラム的に操作できるようになります。
...
OpenPatent ターミナルユーザーインターフェイスを開始します。
...
OpenPatent CLI には次のコマンドもあります。

---

### agent

OpenPatent のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の OpenPatent バックエンドサーバーにターミナルを接続します。

```bash
OpenPatent attach [url]
```

これにより、リモート OpenPatent バックエンドで TUI を使用できるようになります。例えば：
...
OpenPatent は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`OpenPatent auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/OpenPatent/auth.json` に保存されます。

```bash
OpenPatent auth login
```

OpenPatent が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを OpenPatent で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで OpenPatent を実行します。
...
実行中の `OpenPatent serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| `--attach`   |             | 実行中の OpenPatent サーバー (http://localhost:4096 など) に接続します。                    |
...
API アクセスのためにヘッドレス OpenPatent サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
OpenPatent serve
```

これにより、TUI インターフェイスを使用せずに OpenPatent 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `OpenPatent`)。
...
OpenPatent セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは OpenPatent 共有 URL からセッションデータをインポートします。

```bash
OpenPatent import <file>
```

ローカルファイルまたは OpenPatent 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス OpenPatent サーバーを起動します。

```bash
OpenPatent web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて OpenPatent にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `OpenPatent`)。
...
OpenPatent をアンインストールし、関連ファイルをすべて削除します。
...
OpenPatent を最新バージョンまたは特定のバージョンに更新します。
...
OpenPatent CLI は次のグローバルフラグを受け取ります。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `OpenPatent`) をオーバーライドします。 |
