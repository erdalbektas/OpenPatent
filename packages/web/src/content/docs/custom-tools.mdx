---
title: Custom Tools
description: Create tools the LLM can call in OpenPatent.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside OpenPatent's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.OpenPatent/tools/` directory of your project.
- Or globally, by placing them in `~/.config/OpenPatent/tools/`.
...
```ts title=".OpenPatent/tools/bash.ts"
import { tool } from "@OpenPatent-ai/plugin"
