...
Создайте `.OpenPatent/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию OpenPatent или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем OpenPatent [config](/docs/config):
...
- Глобальный: `~/.config/OpenPatent/commands/`
- Для каждого проекта: `.OpenPatent/commands/`
...
OpenPatent включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
