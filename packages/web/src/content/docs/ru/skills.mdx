...
Навыки агента позволяют OpenPatent обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
OpenPatent выполняет поиск в следующих местах:

- Конфигурация проекта: `.OpenPatent/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/OpenPatent/skills/<name>/SKILL.md`.
...
Для локальных путей проекта OpenPatent переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.OpenPatent/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/OpenPatent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.OpenPatent/skills/git-release/SKILL.md` следующим образом:
...
OpenPatent перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `OpenPatent.json`:
...
**Для встроенных агентов** (в формате `OpenPatent.json`):
