...
:::caution
Modes are now configured through the `agent` option in the OpenPatent config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in OpenPatent allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the OpenPatent config.
...
OpenPatent comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.OpenPatent/plans/*.md` to detail the plan itself
...
Configure modes in your `OpenPatent.json` config file:
...
both the global OpenPatent config and the project specific config.
...
Create mode files in `.OpenPatent/modes/` for project-specific modes or `~/.config/OpenPatent/modes/` for global modes:
