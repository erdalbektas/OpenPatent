...
OpenPatent has a list of keybinds that you can customize through `tui.json`.
...
OpenPatent uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The OpenPatent desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `OpenPatent.json`.
