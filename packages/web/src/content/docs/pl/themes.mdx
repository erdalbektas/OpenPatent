...
Dzięki OpenPatent możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie OpenPatent używa naszego własnego motywu `OpenPatent`.
...
OpenPatent ma kilka wbudowanych motywów.
...
- Want OpenPatent to match their terminal's appearance
...
OpenPatent obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/OpenPatent/themes/*.json` lub `$XDG_CONFIG_HOME/OpenPatent/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.OpenPatent/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.OpenPatent/themes/*.json`
