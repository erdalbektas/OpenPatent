...
Umiejętności agenta pozwalają OpenPatent odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
OpenPatent przeszukuje te lokalizacje:

- Project config: `.OpenPatent/skills/<name>/SKILL.md`
- Global config: `~/.config/OpenPatent/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu OpenPatent przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.OpenPatent/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/OpenPatent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.OpenPatent/skills/git-release/SKILL.md` w ten sposób:
...
OpenPatent wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `OpenPatent.json`:
...
**Dla agentów wbudowanych** (w `OpenPatent.json`):
