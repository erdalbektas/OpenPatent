...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji OpenPatent.
...
te lub skonfiguruj własne za pomocą konfiguracji OpenPatent.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.OpenPatent/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `OpenPatent.json`:
...
- Globalnie: `~/.config/OpenPatent/modes/`
...
Jeśli nie ma wpływu na temperaturę, OpenPatent stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna OpenPatent, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.OpenPatent/modes/` dla trybów zapisanych dla projektu lub `~/.config/OpenPatent/modes/` dla trybów globalnych:
