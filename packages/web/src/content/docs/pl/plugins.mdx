---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć OpenPatent.
---

Wtyczki umożliwiające wyłączenie OpenPatent poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania OpenPatent.
...
- `.OpenPatent/plugins/` - Wtyczki na poziomie projektu
- `~/.config/OpenPatent/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/OpenPatent/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/OpenPatent/OpenPatent.json`)
2. Konfiguracja projektu (`OpenPatent.json`)
3. Globalny katalog wtyczek (`~/.config/OpenPatent/plugins/`)
4. Katalog wtyczek projektu (`.OpenPatent/plugins/`)
...
OpenPatent uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z OpenPatent do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do OpenPatent.
...
:::note
Jeśli korzystasz z aplikacji komputerowej OpenPatent, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj OpenPatent czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do OpenPatent:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod OpenPatent. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://OpenPatent.ai/docs/sdk).
