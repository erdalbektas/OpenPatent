---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w OpenPatent.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenPatent używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji OpenPatent.
...
w `~/.local/share/OpenPatent/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim OpenPatent
config.
...
OpenPatent Zen to lista modeli dostarczonych przez zespół OpenPatent, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z OpenPatent. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenPatent Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz OpenPatent i przejdź do [OpenPatent.ai/auth](https://OpenPatent.ai/auth).
...
Działa jak każdy inny dostawca w OpenPatent i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z OpenPatent:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania OpenPatent:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `OpenPatent.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w OpenPatent nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod OpenPatent działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji OpenPatent.
...
OpenPatent używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować OpenPatent
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`OpenPatent.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [OpenPatent-gitlab-auth](https://www.npmjs.com/package/@gitlab/OpenPatent-gitlab-auth).

##### Konfiguracja

Customize through `OpenPatent.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem OpenPatent:
...
Aby używać Google Vertex AI z OpenPatent:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running OpenPatent.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą OpenPatent, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [OpenPatent-helicone-session](https://github.com/H2Shami/OpenPatent-helicone-session), aby automatycznie rejestrować każdą konwersację OpenPatent jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja OpenPatent będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować OpenPatent tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować OpenPatent tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować OpenPatent tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem OpenPatent. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/OpenPatent).
:::
...
Aby korzystać z Ollama Cloud z OpenPatent:
...
3. Skopiuj klucz API do użycia w OpenPatent.
...
6. **Ważne**: Przed użyciem modeli chmurowych w OpenPatent musisz pobrać informacje o modelu lokalnie:
...
OpenPatent Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół OpenPatent. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenPatent Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenPatent Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API OpenPatent.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji OpenPatent.
...
5. Możesz je także dostosować za pomocą konfiguracji OpenPatent. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z OpenPatent:
...
Możesz także dostosować modele za pomocą konfiguracji OpenPatent. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z OpenPatent. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `OpenPatent.json` w katalogu projektu:
...
Pola `limit` pozwalają OpenPatent zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `OpenPatent auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację OpenPatent i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji OpenPatent.
