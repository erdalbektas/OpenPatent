...
Możesz dodać dodatkowe informacje do OpenPatent za pomocą _Model Context Protocol_, w skrócie MCP. OpenPatent obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [OpenPatent Config](https://OpenPatent.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/OpenPatent`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
OpenPatent automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, OpenPatent:
...
Jeśli serwer wymaga uwierzytelnienia, OpenPatent poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `OpenPatent mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji OpenPatent bezpieczne przechowa tokeny w `~/.local/share/OpenPatent/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w OpenPatent, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie OpenPatent, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć OpenPatent z kontem Sentry.
