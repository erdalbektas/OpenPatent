---
title: CLI
description: Opcje i polecenia wiersza poleceń OpenPatent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI OpenPatent domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
OpenPatent
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać OpenPatent programowo.
...
Uruchamia interfejs terminalowy (TUI) OpenPatent.
...
Interfejs CLI OpenPatent zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami OpenPatent.
...
Dołącz terminal do już działającego serwera backendu OpenPatent uruchomionego za pomocą `serve` lub `web`.

```bash
OpenPatent attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem OpenPatent. Na przykład:
...
OpenPatent integruje się z wieloma dostawcami. Możesz użyć `OpenPatent auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/OpenPatent/auth.json`.

```bash
OpenPatent auth login
```

Podczas uruchamiania OpenPatent ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w OpenPatent.
...
Uruchom OpenPatent w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `OpenPatent serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| `--attach`   |       | Dołącz do działającego serwera OpenPatent (np. http://localhost:4096)     |
...
Uruchom serwer OpenPatent (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
OpenPatent serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API OpenPatent bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `OpenPatent`).
...
Lista wszystkich sesji OpenPatent.
...
Pokaż statystyki wykorzystania tokenów i sesji OpenPatent.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania OpenPatent.
...
Uruchom serwer OpenPatent z interfejsem internetowym.

```bash
OpenPatent web
```

Uruchamia to serwer HTTP i udostępnia OpenPatent przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `OpenPatent`).
...
Odinstaluj OpenPatent i usuń wszystkie powiązane pliki.
...
Aktualizuj OpenPatent do najnowszej lub określonej wersji.
...
Interfejs CLI OpenPatent przyjmuje następujące flagi globalne dla każdego polecenia.
...
OpenPatent można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `OpenPatent`)     |
