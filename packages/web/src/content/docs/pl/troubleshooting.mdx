...
Aby debugować problemy z OpenPatent, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\OpenPatent\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `OpenPatent --log-level DEBUG`.

---

## Przechowywanie danych

OpenPatent przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/OpenPatent/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\OpenPatent`
...
OpenPatent Desktop uruchamia w tle lokalny serwer OpenPatent (sidecar `OpenPatent-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OpenPatent` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/OpenPatent/OpenPatent.jsonc` (lub `~/.config/OpenPatent/OpenPatent.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/OpenPatent/OpenPatent.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\OpenPatent\OpenPatent.jsonc`
...
OpenPatent może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/OpenPatent/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\OpenPatent\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby OpenPatent mógł ją odbudować.

1. Całkowicie zamknij OpenPatent Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/OpenPatent`
- **Linux**: usuń `~/.cache/OpenPatent` (lub uruchom `rm -rf ~/.cache/OpenPatent`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\OpenPatent`

3. Uruchom ponownie OpenPatent Desktop.

---

### Napraw problemy z połączeniem z serwerem

OpenPatent Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `OpenPatent.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows OpenPatent Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami OpenPatent.

---

### Brak powiadomień

OpenPatent Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla OpenPatent w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie OpenPatent Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji OpenPatent Desktop):

- `OpenPatent.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `OpenPatent.global.dat` i `OpenPatent.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z OpenPatent:
...
3. Upewnij się, że masz najnowszą wersję z `OpenPatent upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `OpenPatent models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\OpenPatent`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. OpenPatent dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\OpenPatent`

2. Uruchom ponownie kod OpenPatent, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to OpenPatent do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
OpenPatent wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
