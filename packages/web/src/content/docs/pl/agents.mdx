...
W OpenPatent obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
OpenPatent zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
OpenPatent ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

OpenPatent ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `OpenPatent.json`:
...
- Globalnie: `~/.config/OpenPatent/agents/`
- W projekcie: `.OpenPatent/agents/`
...
Jeśli nie zostanie skonfigurowana, OpenPatent używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji OpenPatent, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji OpenPatent w `provider/model-id`. Na przykład, [OpenPatent Zen](/docs/zen), użyj `OpenPatent/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `OpenPatent models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/OpenPatent).
:::
