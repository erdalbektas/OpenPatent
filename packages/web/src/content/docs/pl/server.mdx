---
title: Serwer
description: Interakcja z serwerem OpenPatent za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `OpenPatent serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient OpenPatent.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `OpenPatent.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `OpenPatent` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `OpenPatent serve`, jak i `OpenPatent web`.
...
Kiedy uruchomisz `OpenPatent`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera OpenPatent do programowej interakcji z OpenPatent.
:::

Ta architektura umożliwia obsługę wielu klientów przez OpenPatent i programową interakcję z kodem otwartym.

Możesz uruchomić `OpenPatent serve`, aby uruchomić samodzielny serwer. Jeśli masz
OpenPatent TUI działa, `OpenPatent serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki OpenPatent [IDE](/docs/ide).
...
Serwer OpenPatent udostępnia następujące interfejsy API.
