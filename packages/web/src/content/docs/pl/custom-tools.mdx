...
Narzędzia stosowane do funkcji, z których LLM może korzystać podczas rozmów. Współpracują z [wbudowanymi narzędziami] (./tools) OpenPatent, wtyczka jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.OpenPatent/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/OpenPatent/tools/`.
